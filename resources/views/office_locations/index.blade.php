...
<div class="content">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1">Office Locations</h4>
            <p class="text-muted mb-0">Manage office locations</p>
        </div>
        <div>
            <a href="{{ route('office-locations.import') }}" class="btn btn-success me-2">
                <i class="ph-upload me-2"></i>Import
            </a>
            <a href="{{ route('office-locations.create') }}" class="btn btn-primary">
                <i class="ph-plus me-2"></i>Add Office Location
            </a>
        </div>
    </div>
    @if(session('success'))
    ...
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0">All Office Locations</h5>
        </div>

        <div class="card-body">
            <table class="table eagle-table2 table-hover" id="office-locations-table">
                <thead>
                    <tr>
                        <th>Code</th>
                        <th>Name</th>
...
<script>
    $(document).ready(function() {
        $('#office-locations-table').DataTable({
            processing: true,
            serverSide: true,
    ...
    function deleteLocation(id) {
        if (confirm('Are you sure you want to delete this office location?')) {
            ...
            form.action = '{{ url('office-locations') }}/' + id;
            form.submit();
        }
    }
</script>
